...
npx skills add agntdev/skills/tree/v0.16.1
...
The skill bundle is not versioned in the npm sense. We tag the `agntdev-skills` git repo (`v0.14.0`, `v0.14.1`, `v0.14.2`, `v0.14.3`, `v0.15.0`, `v0.16.0`, `v0.16.1`, `v0.16.2`, ...) and the tag-scoped `npx skills add` URL is the install contract. The CLI (`@agntdev/cli`) has its own version; the skills and CLI share a major.minor by convention but the patch is independent — `agnt-cli@0.16.0` (CLI) + `v0.16.2` (skills) is the current pair.
